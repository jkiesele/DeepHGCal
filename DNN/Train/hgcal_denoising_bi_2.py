...
def denoising_model(Inputs, nclasses, nregressions, dropoutRate=0.05, momentum=0.6):
    ...
    lstm_1 = LSTM(30, return_sequences=True, input_shape=(L, 30))
    lstm_2 = Bidirectional(LSTM(30, return_sequences=True, input_shape=(L, 30)))

    x = lstm_1(x)
    x = lstm_2(x)
    x = ReshapeBatch((-1, 13, 13, L, 60))(x)
    x = Conv3D(6, kernel_size=(1, 1, 1), padding='same', activation=None,
                      kernel_initializer='lecun_uniform', kernel_regularizer=l2(l2_lambda))(x)
