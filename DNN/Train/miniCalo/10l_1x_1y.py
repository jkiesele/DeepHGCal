...
def resnet_like_3D(Inputs,nclasses,nregressions,dropoutRate=0.05,momentum=0.6, nodemulti=32, l2_lambda=0.0001, use_dumb_bias=False):
    ...
    x = miniCalo_preprocess(Inputs[0],"10l_1x_1y_pre")
    #x = Inputs[0]
    x = Flatten()(x)
    id = Dense(1, trainable=False,name = '10l_1x_1y_id')(x)
    
    print(x.shape)
    
    

    x = Dense(64,name="pre_E_a1")(x)
    x = LeakyReLU(alpha=0.01)(x)
    x = Dense(64,name="pre_E_b1")(x)
    x = LeakyReLU(alpha=0.01)(x)
    x = Dense(1,name="pre_E_c1" ,use_bias=False)(x)
...
learn.append(Learning_sched(lr=1e-2,     nepochs=2,    batchsize=1000,  funccall=None, loss_weights=[1e-7, 1.],  en_loss='mean_squared_error'))
learn.append(Learning_sched(lr=3e-4,     nepochs=1,    batchsize=5000,  funccall=None, loss_weights=[1e-7, 1.],  en_loss=calo_loss_rel))
learn.append(Learning_sched(lr=1e-4,     nepochs=1,   batchsize=5000,  funccall=None, loss_weights=[1e-7, 1.],  en_loss=calo_loss_rel))
learn.append(Learning_sched(lr=1e-5,     nepochs=2,   batchsize=15000,  funccall=None, loss_weights=[1e-7, 1.],  en_loss=calo_loss_rel))
learn.append(Learning_sched(lr=1e-6,     nepochs=3,   batchsize=15000,  funccall=None, loss_weights=[1e-7, 1.],  en_loss=calo_loss_rel))
learn.append(Learning_sched(lr=1e-7,     nepochs=4,   batchsize=15000,  funccall=None, loss_weights=[1e-7, 1.],  en_loss=calo_loss_rel))
