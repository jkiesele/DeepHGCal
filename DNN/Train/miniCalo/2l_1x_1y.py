...
def resnet_like_3D(Inputs,nclasses,nregressions,dropoutRate=0.05,momentum=0.6, nodemulti=32, l2_lambda=0.0001, use_dumb_bias=False):
    ...
    x = miniCalo_preprocess(Inputs[0],"2l_1x_1y_pre")
    #x = Inputs[0]
    x = Flatten()(x)
    id = Dense(1, trainable=False,name = '2l_1x_1y_id')(x)
    
    print(x.shape)
    
    

    x = Dense(64,name="pre_E_a")(x)
    x = LeakyReLU(alpha=0.01)(x)
    x = Dense(1,name="pre_E_c" ,use_bias=False)(x)
