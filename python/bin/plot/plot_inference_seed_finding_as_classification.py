...
with open(os.path.join(config['test_out_path'], 'inference_output_files.txt')) as f:
    ...
    for i in content:
        with gzip.open(i.strip()) as f:
            ...
            for j in data:
                try:
                    input, num_entries, output, seed_indices = j
                except ValueError:
                ...
                predicted_indices = np.argmax(output, axis=0)
                if np.sum(predicted_indices == np.flip(seed_indices, axis=0)) > np.sum(predicted_indices == seed_indices):
                    seed_indices = np.flip(seed_indices, axis=0)

                is_correct_vector = seed_indices == predicted_indices

                energy_values.append(shower_sum_energy_1)
                correct.append(is_correct_vector[0])
                energy_values.append(shower_sum_energy_2)
                correct.append(is_correct_vector[1])
...
def acc_2d_plot(energy_values, correct):
    ...
    for i in range(len(energy_values_1)):
        corr_2d[bin_indices_y[i], bin_indices_x[i]] += int(correct[i*2])
        corr_2d[bin_indices_x[i], bin_indices_y[i]] += int(correct[i*2+1])
...
cax = ax.imshow(corr_2d, interpolation='nearest', extent=extent, vmin=0, vmax=1)
...
for y_index, y in enumerate(y_positions):
    for x_index, x in enumerate(x_positions):
        label = ('%.2f' % corr_2d[size -1- y_index, x_index])
        text_x = x + jump_x
        text_y = y + jump_y
