...
class SparseConv5(SparseConv):

    def __init__(self, n_space, n_space_local, n_all, n_max_neighbors, batch_size, max_entries, num_classes,
                 learning_rate=0.0001):
        super(SparseConv5, self).__init__(n_space, n_space_local, n_all, n_max_neighbors, batch_size, max_entries,
    ...
    def _find_logits(self):
        ...
        net = sparse_conv(net, num_neighbors=9, output_all=20)
        net = sparse_conv(net, num_neighbors=9, output_all=20)
        net = sparse_conv(net, num_neighbors=9, output_all=20)
        net = sparse_max_pool_factored(net, 3)
        net = sparse_conv(net, num_neighbors=9, output_all=40)
        net = sparse_max_pool_factored(net, 2)
        net = sparse_conv(net, num_neighbors=9, output_all=50)
        net = sparse_max_pool_factored(net, 2)
        net = sparse_conv(net, num_neighbors=9, output_all=60)
